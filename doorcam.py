...
logging.basicConfig(
    filename='/home/am/projects/opencv_examples/doorcam.log',
    filemode='a',
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level = logging.INFO
)

# touch file to make sure program is running
os.system("touch /home/am/projects/opencv_examples/doorcam.txt")
...
client = InfluxDBClient(config.INFLUX_DB_ADDRESS, config.INFLUX_DB_PORT)
client.switch_database(config.INFLUX_TABLE)

def open_close_connection(f):
    conn = ps.connect(f"dbname={config.POSTGRES_DB} user={config.POSTGRES_USER}")
    cur = conn.cursor()
    f()
    cur.close()
    conn.close()
